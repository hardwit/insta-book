...
export default class PDFService {
    ...
    return new Promise(resolve => {
      ...
      data.forEach((photo, index) => {
        ...
        if (!photo.carouselMedia) {
          ...
          const promises = photo.carouselMedia.map(photo =>
            request({
              url: photo.imageUrl,
              encoding: null
            })
          ...
          Promise.all(promises).then(results => this.createPdfFromCarousel(doc, results, photo))
        }
      })
    })
  }
  ...
  createPdfFromCarousel(doc, pictures, photo) {
    const date = moment(photo.date).format('LL')
    const formatedDate = date.slice(0, date.length - 3)
